...
Write-Host "Connecting to ${user}@${serverHost} and running full refresh (wipe, clone, deploy)..." -ForegroundColor Cyan
(Get-Content $scriptPath -Raw) -replace "`r`n", "`n" | ssh -i $keyPath -o StrictHostKeyChecking=no "${user}@${serverHost}" "bash -s"
